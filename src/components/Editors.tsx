...
export const OperationsEditor = ({ settings, onChange }: EditorProps) => {
  ...
  return <>
    ...
    <Panel title="基本営業条件" caption="MVPでは月営業日数を基準に期間集計します。曜日別設定は将来の実カレンダー計算用に保持します。">
      <div className="form-grid form-grid-3">
        ...
        <NumberField label="月営業日数" suffix="日" min={1} max={31} value={settings.business.operatingDaysPerMonth} onChange={(event) => updateBusiness('operatingDaysPerMonth', numberValue(event.target.value))} />
        <NumberField label="営業開始" type="time" value={settings.business.openingTime} onChange={(event) => updateBusiness('openingTime', event.target.value)} />
        <NumberField label="営業終了" type="time" value={settings.business.closingTime} onChange={(event) => updateBusiness('closingTime', event.target.value)} />
        <NumberField label="1日営業時間" suffix="時間" min={0} max={24} value={settings.business.hoursPerDay} onChange={(event) => updateBusiness('hoursPerDay', numberValue(event.target.value))} hint="休憩・中休みを除く実稼働時間" />
      </div>
    </Panel>
    <Panel title="曜日別営業カレンダー（拡張設定）" caption="現在の集計は月営業日数を使用します。曜日設定は保存され、将来の実日付計算へ引き継げます。">
...
const SourceEditor = ({ settings, sources, onChange, allowOutputs = true }: { settings: AppSettings; sources: SourceRef[]; onChange: (sources: SourceRef[]) => void; allowOutputs?: boolean }) => {
  ...
  return <div className="source-editor">
    {sources.map((source, index) => <div className="source-row" key={`${source.sourceType}-${source.sourceId}-${index}`}>
      ...
      <span className="input-shell compact"><input type="number" step="any" min="0" value={source.quantity} onChange={(event) => onChange(sources.map((item, itemIndex) => itemIndex === index ? { ...item, quantity: numberValue(event.target.value) } : item))}/><span>{source.unit}</span></span>
...
export const LaborEditor = ({ settings, onChange }: EditorProps) => {
  ...
  const operatingDaily = settings.labor.reduce((sum, role) => sum + role.hourlyWage * role.headcount * role.hoursPerDay, 0)
  const prepDaily = simulate(settings, 'day').costs.prepLabor
  ...
  return <>
    ...
    <div className="editor-summary"><div><span>営業人件費 / 日</span><strong>{formatYen(operatingDaily)}</strong></div><div><span>仕込み人件費 / 日</span><strong>{formatYen(prepDaily)}</strong></div><div><span>人件費合計 / 月</span><strong>{formatCompactYen((operatingDaily + prepDaily) * settings.business.operatingDaysPerMonth)}</strong></div></div>
    <Panel title="スタッフ・役割" caption="限界人件費率は、将来『仕込みをやめてもシフトが変わらない』ケースを比較するため保持します。">
    ...
    <div className="calculation-note"><Icon name="info" size={17}/><span>仕込み人件費は工程ごとに「実作業時間 × 担当役割の時給」で計算します。工程所要時間（待ち時間を含む）は人件費へ直接加算しません。</span></div>
  </>
}
...
const UtilitySection = ({ title, unit, config, dailyQuantity, onChange }: { title: string; unit: string; config: UtilityConfig; dailyQuantity: number; onChange: (config: UtilityConfig) => void }) => (
  <Panel title={title} caption={`1日使用量 ${formatNumber(dailyQuantity, 2)} ${unit} · ${formatYen(dailyQuantity * config.unitPrice)}`} actions={config.isReferencePrice ? <Badge tone="reference">初期参考値</Badge> : undefined}>
...
export const UtilitiesEditor = ({ settings, onChange }: EditorProps) => {
  ...
  return <>
    ...
    <div className="utility-kpis">
      <div><i className="water"/><span>水道 / 日</span><strong>{formatYen(calculateUtilityQuantity(settings.utilities.water, meals, hours) * settings.utilities.water.unitPrice)}</strong></div>
      <div><i className="gas"/><span>ガス / 日</span><strong>{formatYen(calculateUtilityQuantity(settings.utilities.gas, meals, hours) * settings.utilities.gas.unitPrice)}</strong></div>
      <div><i className="electric"/><span>電気 / 日</span><strong>{formatYen(calculateUtilityQuantity(settings.utilities.electricity, meals, hours) * settings.utilities.electricity.unitPrice)}</strong></div>
      <div><i className="oil"/><span>揚げ油 / 日</span><strong>{formatYen(oilLiters * settings.fryingOil.unitPricePerL)}</strong></div>
    </div>
...
export const ComparisonEditor = ({ settings, onChange }: EditorProps) => {
  const result = useMemo(() => compareMakeBuy(settings), [settings])
  const [scenarioB, setScenarioB] = useState({ meals: 110, hours: 10, days: 20 })
  const scenarioAResult = simulate(settings, 'month')
  const scenarioBSettings: AppSettings = { ...settings, business: { ...settings.business, mealsPerDay: scenarioB.meals, hoursPerDay: scenarioB.hours, operatingDaysPerMonth: scenarioB.days } }
  ...
  return <>
    <PageTitle eyebrow="MAKE OR BUY" title="内製 vs 既製品" description="材料・人件費・水道光熱・廃棄・作業時間を揃え、同じ用途の調達方法を比較します。" />
    <div className="comparison-hero">
      ...
      <div className="roi-display"><span>内製ROI</span><strong>{formatYen(result.savingsPerWorkHour)}</strong><small>削減額 / 追加作業1時間</small></div>
    ...
    <div className="dashboard-grid comparison-details">
      <Panel title="内製の単位原価内訳" caption="工程のバッチ丸めを外した、比較用の連続単価です。">
        <div className="breakdown-list">
          {([
            ['原材料', result.homemadeBreakdown.prepMaterials], ['実作業人件費', result.homemadeBreakdown.prepLabor], ['水道', result.homemadeBreakdown.water],
    ...
    <PageTitle eyebrow="SCENARIO" title="営業シナリオ比較" description="現在の店舗設定をAとし、食数・営業時間・営業日数を変えたBを月次で比較します。" />
    <Panel className="scenario-panel">
      <div className="scenario-headings"><div><Badge>SCENARIO A</Badge><h3>現在の設定</h3><p>{settings.business.mealsPerDay}食 · {settings.business.hoursPerDay}時間 · 月{settings.business.operatingDaysPerMonth}日</p></div><div><Badge tone="positive">SCENARIO B</Badge><h3>比較案</h3><div className="scenario-inputs"><label><input type="number" value={scenarioB.meals} onChange={(event) => setScenarioB({ ...scenarioB, meals: numberValue(event.target.value) })}/>食</label><label><input type="number" value={scenarioB.hours} onChange={(event) => setScenarioB({ ...scenarioB, hours: numberValue(event.target.value) })}/>時間</label><label>月<input type="number" value={scenarioB.days} onChange={(event) => setScenarioB({ ...scenarioB, days: numberValue(event.target.value) })}/>日</label></div></div></div>
      <div className="scenario-table">
        ...
        {([
          ...
          ['人件費', scenarioAResult.costs.operatingLabor + scenarioAResult.costs.prepLabor, scenarioBResult.costs.operatingLabor + scenarioBResult.costs.prepLabor],
